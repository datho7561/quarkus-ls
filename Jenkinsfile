    ...
    success {
      script {
        withCredentials([string(
            credentialsId: 'rhdevelopersci-github-token',
            variable: 'GIT_PASSWORD')]) {
          sh '''
            if [[ ${VERSION} != "" ]]; then
              cd ${WORKSPACE}
              git tag '${VERSION}'
              git push git://ghost:${GIT_PASSWORD}@github.com/datho7561/quarkus-ls.git '${VERSION}' 
              git push git://ghost:${GIT_PASSWORD}@github.com/datho7561/quarkus-ls.git ${GIT_LOCAL_BRANCH}
            fi
          '''
        }
      }
